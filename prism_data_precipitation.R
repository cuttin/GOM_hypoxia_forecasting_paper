...
options(prism.path = "C:/Temp/prism/")
get_prism_monthlys(type = "ppt", years = c(1980:2016), mon = c(1:12), keepZip = T) # download data 
#get_prism_monthlys(type = 'ppt', years = 1999:2000, mon = 2, keepZip = T) # download data 
ls_prism_data(name = TRUE) # see the list of stats
temp <- ls_prism_data(name = TRUE)
temp$product_name
# create a dataframe with year-month from 1980 to 2016
monthly_data <- data.frame(Date = seq.Date(from = as.Date("1980-01-01"), 
                                           to = as.Date("2016-12-31"), by = "month")) %>%
  mutate(year = year(Date), month = month(Date)) %>% 
  select(- Date)
monthly_data$p <- NA
## Get mean monthly precipitation
for (i in seq_along(temp$files)) {
  ...
  monthly_data$p[i] <- mean(d$ppt) # write to dataframe
  print(i)
}

glimpse(monthly_data)
write_csv(monthly_data, "./data/prism_prec_monthly.csv") ## all data

win <- monthly_data %>% 
  filter(month %in% c(1:3)) %>%
  group_by(year) %>%
  summarise(jfm_P = mean(p, na.rm = T))  
spr <- monthly_data %>% 
  filter(month %in% c(4:5)) %>%
  group_by(year) %>%
  summarise(am_P = mean(p, na.rm = T))
may <- monthly_data %>% 
  filter(month %in% c(5)) %>%
  group_by(year) %>%
  summarise(m_P = mean(p, na.rm = T))
precip <- left_join(win, spr, by = "year") %>% 
  left_join(., may, by = "year")
write_csv(precip, "./data/prism_precip_monthly_watershed.csv")
###################
###################
###################
###################
...
plot1 <- ggplot(data = dff, aes(x = longitude, y = latitude))+
  ...
  labs(color = "Station within watershed: 0-out, 1-in")
#ggsave(plot1, "checkit.png")
