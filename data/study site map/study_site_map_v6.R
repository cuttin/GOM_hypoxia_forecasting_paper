...
pacman::p_load(adaptMCMC, TTR, truncnorm, tidyverse, lubridate, suncalc, data.table, deSolve,
               ...
               sf, ggforce, rnaturalearth, rnaturalearthdata, ggspatial, rnaturalearthhires)
select <- dplyr::select
...
la_border <- filter(us_states, abbreviation == "LA")  
line1 <- la_border %>% filter(long < -93.8)  
map <- map_data('state')
unique(map$region)
la <- subset(map, region %in% "louisiana")
...
boundary <- maps::map("state", boundary=FALSE, col="gray")
boundary[4]
bdf <- as.data.frame(boundary )
class(boundary)
glimpse(boundary)
bdf <- st_as_sf(maps::map("state", fill=TRUE, plot =FALSE, boundary=FALSE, interior = F))
#################################################################################################
...
#base_map <- "https://api.mapbox.com/styles/v1/cuttin/ck1xx6zdq0v331bmrms8mny9g/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoiY3V0dGluIiwiYSI6ImNrMWpsZXUxdjAyODUzYnI0dWhpZ2dubXoifQ.6TmmPa9bXBqfDgPa3TNb0A"
#base_map <- "https://api.mapbox.com/styles/v1/cuttin/ckwcd3dyd23ti14jvjfgvxf7c/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoiY3V0dGluIiwiYSI6ImNrMWpsZXUxdjAyODUzYnI0dWhpZ2dubXoifQ.6TmmPa9bXBqfDgPa3TNb0A"
base_map <- "https://api.mapbox.com/styles/v1/cuttin/ckwcgmkjf2ibq14mn132we2vl/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoiY3V0dGluIiwiYSI6ImNrMWpsZXUxdjAyODUzYnI0dWhpZ2dubXoifQ.6TmmPa9bXBqfDgPa3TNb0A"
sr <- "+proj=longlat"
...
###
myMaposmd_2 <- OpenStreetMap::openmap(upperLeft = c(latext[2], lonext[2]),
                                      lowerRight = c(latext[1]-0.2, lonext[1]),
                                      type = base_map,zoom = 9.1)
myMaposmd_2 <- OpenStreetMap::openproj(myMaposmd_2, projection = sr)
#################################################################################################
...
poly_west <- data.frame(st_coordinates(polygon_west[,1])) %>% dplyr::select(lon = X, lat = Y) %>% 
  mutate(section = "West")
max(poly_west$lon)
poly_west %>% filter(lon > max(poly_west$lon-0.02))
ggplot(poly_west, aes(lon, lat))+geom_point(shape = 1)+
  geom_point(data = poly_west %>% filter(lon > max(poly_west$lon-0.02)), aes(lon, lat), color = "red")
poly_west <- poly_west %>% 
  mutate(lon = ifelse(lon > max(poly_west$lon-0.02), -91.305, lon),
         lat = ifelse(lat > 29.2, lat-0.05, lat))
###
pnts_east <- depth_df %>%
  filter(ShelfSect == 2) %>%
  sf::st_as_sf(coords = c("lon", "lat"), crs = 4326)
...
poly_east <- data.frame(st_coordinates(polygon_east[,1])) %>% dplyr::select(lon = X, lat = Y) %>% 
  mutate(section = "East")
min(poly_east$lon)
poly_east %>% filter(lon < min(poly_east$lon+0.02))
ggplot(poly_east, aes(lon, lat))+geom_point(shape = 1)+
  geom_point(data = poly_east %>% filter(lon < min(poly_east$lon+0.02)), aes(lon, lat), color = "red")
poly_east <- poly_east %>% 
  mutate(lon = ifelse(lon < min(poly_east$lon+0.02), -91.300, lon))
###
...
#################################################################################################
# Grid from depth
grid <- with(depth_df, interp(x = lon, y = lat, z = RASTERVALU*-1, linear = TRUE, extrap = TRUE,
...
glimpse(la)
# Finally plot
gommap <- #OpenStreetMap::autoplot.OpenStreetMap(myMaposmd_2, expand=FALSE)+
  ggplot(data = world) +
  geom_sf() +
  geom_sf(data = bdf, position = "identity")+
  coord_sf(ylim = c(latext[1]-0.2, 30.7), 
           xlim = c(lonext[2]-0.15, lonext[1]+0.3), expand = FALSE)+
  #maps::map("state", boundary=FALSE, col="gray")+
  ...
  geom_polygon(data = poly, aes(lon, lat, fill = section), alpha = 0.5)+ # , color = section size = 2.5
  #geom_polygon(data = la,
   #            mapping = aes(x = long, y = lat,
    #                         group = group), color = "grey20", size = 0.1, fill = NA)+
  
  #usmap::plot_usmap(include = "LA")+
  #borders("state")+
  theme_bw()+
  ...
  geom_contour(data = dfll, aes(x = x, y = y, z = z), 
               breaks = c(10, 20, 30, 40, 50, 60, 70), linetype = "dashed", color = "grey50")+
  ...
  #annotation_custom(g_arrow, xmin = -93, xmax = -90, 
  #                  ymin = 29, ymax = 30.5)+
  theme(legend.position = c(1, 1), #c(0, 1) "top"
        legend.justification = c(1, 1),
        strip.background = element_blank(),
        #legend.background = element_blank(),
        legend.background = element_rect(fill = "white", color = "black", size=0.4),
        legend.title = element_blank(),
        #legend.margin=margin(0.0,0.05,0.05,0.1),
        #legend.box.margin=margin(1.5,1.5,1.5,1.5),
  ...
  #annotation_custom(g_arrow, xmin = (lonext[1]-0.3), xmax = (lonext[1]-0.1), 
  #                  ymin = (latext[1]), ymax = (latext[1]+0.5))+
  #scalebar(x.min = (lonext[2]), x.max = (lonext[1]-0.2),
...
us <- ggplot()+ 
  ...
  geom_point(data = dff, aes(x = longitude, y = latitude, group = 1), color = 'lightblue', alpha = 0.2, size = 0.4)+
  ...
  theme_minimal()+
  geom_polygon(data = poly, aes(lon, lat, fill = section), alpha = 0.5)+
  scale_fill_manual(values = cbbPalette[c(2, 4)])+
  geom_rect(mapping = aes(xmin = lonext[2]-1, xmax = lonext[1]+1, 
                          ymin = latext[1]-1, ymax = latext[2]+1), 
            color = cbbPalette[c(7)], fill = NA, size = 0.8)+
  ...
  theme(plot.margin = unit(c(-0.1,-0.5,-0.1,-0.5), "cm"), #t = 0, r = 0, b = 0, l = 0
    ...
    plot.background = element_rect(colour = "black", fill="white", size=0.4))
us
########################################################################################
########################################################################################
########################################################################################
finplot <- gommap + annotation_custom(ggplotGrob(us), 
                                      xmin = -Inf, xmax = lonext[2]+1.5, #lonext[2]+0.05
                                      ymin = latext[2]-1.4, ymax = latext[2]-0.06) # latext[2]-0.1
finplot
#ggpubr::ggarrange(us, gommap, nrow = 2, ncol = 1)

(date_now <- Sys.Date())
ggsave(filename = paste0('Q:\\My Drive\\research\\transfered from Katin\\GOM model\\Clean\\data\\study site map\\fig_1_v6_', date_now, '.pdf'),  
       plot = finplot,   
       dpi = 500, width = 20, height = 10, units = c("cm"))
#######################################################################################
ggsave(filename = paste0('Q:\\My Drive\\research\\transfered from Katin\\GOM model\\Clean\\data\\study site map\\fig_1_v6_', date_now, '.png'),  
       plot = finplot,   
       dpi = 500, width = 20, height = 10, units = c("cm"))
