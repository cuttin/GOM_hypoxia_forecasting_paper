...
ggplot() + 
  ...
  coord_sf()
str(contours)
contours_df <- as.data.frame(sf::st_coordinates(sf::st_transform((contours), crs = sr))) 
glimpse(contours_df)
hist(contours_df$L1)
## Depth
depth <- sf::st_read('Q:\\My Drive\\research\\transfered from Katin\\GOM model\\Clean\\data\\study site map\\EstPointDepth2.shp')
ggplot() + 
  geom_sf(data = depth %>% filter(RASTERVALU > 0), size = 1, color = "black") + 
  ggtitle("Mis & Atch") + 
  coord_sf()
str(depth)
summary(depth$RASTERVALU)

unique(depth$ShelfSect)
depth_df <- as.data.frame(sf::st_coordinates(sf::st_transform(depth, crs = sr))) 
...
us <- ggplot()+ 
  ...
  geom_rect(mapping = aes(xmin = lonext[2]-1, xmax = lonext[1]+1, 
                          ymin = latext[1]-0.5, ymax = latext[2]-0.5), 
            color = cbbPalette[c(7)], fill = NA, size = 0.7)+
...
(date_now <- Sys.Date())
ggsave(filename = paste0('Q:\\My Drive\\research\\transfered from Katin\\GOM model\\Clean\\data\\study site map\\fig_1_v7_', date_now, '.pdf'),  
       plot = finplot,   
       dpi = 500, width = 20, height = 10, units = c("cm"))
#######################################################################################
ggsave(filename = paste0('Q:\\My Drive\\research\\transfered from Katin\\GOM model\\Clean\\data\\study site map\\fig_1_v7_', date_now, '.png'),  
       plot = finplot,   
       dpi = 500, width = 20, height = 10, units = c("cm"))
