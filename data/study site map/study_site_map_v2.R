...
#################################################################################################
(latext <- c(min(lonlatdf$lat) + 0.1, 30.7)) #max(lonlatdf$lat) + 1.5))
...
gommap <- OpenStreetMap::autoplot.OpenStreetMap(myMaposmd, expand=FALSE)+
  ...
  geom_point(data = misatch_df %>% filter(Y < 30.7, Y > 29.5), aes(X, Y, group = L1), color = cbbPalette[c(6)], size = 0.7)+
...
us <- ggplot()+ 
  ...
  geom_rect(mapping = aes(xmin = lonext[1]+1, xmax = lonext[2], 
            ...
            color = cbbPalette[c(7)], fill = NA, size = 1.5)+
  ...
  theme(plot.margin = unit(c(0.1,0.1,-0.15,0.1), "cm"))
us
########################################################################################
########################################################################################
########################################################################################
...
(date_now <- Sys.Date())
ggsave(filename = paste0('Q:\\My Drive\\research\\transfered from Katin\\GOM model\\Clean\\data\\study site map\\fig_1_', date_now, '.pdf'),  
       plot = ggpubr::ggarrange(us, gommap, nrow = 2, ncol = 1),   
       dpi = 500, width = 20, height = 20, units = c("cm"))
#######################################################################################
