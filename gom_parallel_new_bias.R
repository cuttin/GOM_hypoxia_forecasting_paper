...
glimpse(reg_input)
#saveRDS(reg_input, "./data/reg_input.RDS")
## load_models
...
bias_pred <- readRDS(file = "./data/bias_correction_new.rds")
...
mod_bias <- readRDS("./data/bias_regression_new.rds")
summary(mod_bias)
backward_seq <- seq(from = 29, to = 0)
(bias_sims <- append(backward_seq, rep(NA, 92)))
length(bias_sims)
...
for (i in 1:N) {
sims <- 1 + (coef(mod_bias)[1] + 
               rnorm(n = 1, mean = 0, sd = sqrt(diag(vcov(mod_bias)))[1])) * bias_sims 
...
ha_out <- data.frame()
years_of_interest <- years_of_calibration
years_of_interest <- missing_years
N <- 1000
########################################################################################
...
foreach::foreach(j = seq_along(years_of_interest),
  ...
  bias_1000_sims_all <- rep(1, sim_num_all)  + 
    (rep(coef(mod_bias)[1], sim_num_all) + 
       rnorm(n = sim_num_all, mean = 0, sd = sqrt(diag(vcov(mod_bias)))[1])) * rep(bias_sims, sim_num_all) 
  ...
  bias_1000_sims_par <- rep(1, sim_num_par)  + 
    (rep(coef(mod_bias)[1], sim_num_par) + 
       rnorm(n = sim_num_par, mean = 0, sd = sqrt(diag(vcov(mod_bias)))[1])) * rep(bias_sims, sim_num_par)
  ...
  saveRDS(out_ha_all, paste0("./data/all_sim_09-09_paral/ha_new_", yy, ".rds"))
  saveRDS(temp_bwdo,  paste0("./data/all_sim_09-09_paral/bwdo_new_", yy, ".rds"))
...
bias_1000_sims_hm <- rep(1, sim_num_hm)  + 
  (rep(coef(mod_bias)[1], sim_num_hm) + 
     rnorm(n = sim_num_hm, mean = 0, sd = sqrt(diag(vcov(mod_bias)))[1])) * rep(bias_sims, sim_num_hm) 
...
bias_sims_all <- rep(1, length(sim_num))  + 
  (rep(coef(mod_bias)[1], length(sim_num)) + 
     rnorm(n = length(sim_num), mean = 0, sd = sqrt(diag(vcov(mod_bias)))[1])) * rep(bias_sims, length(sim_num) / 122) 
